...
function setListener(db) {
    ...
    submitButton.addEventListener("click", ev => {
        ...
        if(allFieldsPassTests()) {
            ...
            console.log(ISBNField.value);
            libraryRef.where("ISBN", "==", ISBNField.value)
                .get()
                .then(function (querySnapshot) {
                    querySnapshot.forEach(function (doc) {
                        console.log(doc.id, " => ", doc.data());
                        // Build doc ref from doc.id
                        let docID = db.collection("library").doc(doc.id);
                        docID.update({semestercode: semesterCodeField.value});
                        docID.update({coursenumber: courseNumberField.value});
                        docID.update({coursename: courseNameField.value});
                        docID.update({courseleadcode: courseLeadCodeField.value});
                        docID.update({courseleadname: courseLeadNameField.value});
                        docID.update({title: titleField.value});
                        docID.update({author: authorField.value});
                        docID.update({notes: notesField.value});
                    })
                });
